...
description: >
  ...
  identical and sacred — the five-tone warm-stone palette, -apple-system 400 + -apple-system, the
...
colors:
  # Hand-corrected after build-frame.mjs. The automatic remix assigned the product's
  # SEMANTIC colors to decorative roles — line became #0d68d3 (which means "Agent 自主决定"
  # inside the product) and accent became #b72a24 (which means "约束冲突"). In this video the
  # blue / red / green only ever appear inside real product screenshots, where they carry
  # their real meaning. The frame layer is paper + ink + one warm grain, nothing else.
  bg-primary: "#F3EFE6"      # 欢迎页纸色 --paper — the film's ground
  bg-secondary: "#EAE4D7"    # a half-step deeper paper for matte bands
  text-primary: "#28382F"    # 欢迎页墨色 --paper-ink
  text-secondary: "#6B7268"  # muted ink on paper
  accent: "#A86A3C"          # 纸墨粒子里的暖色颗粒 — scarce, never a UI meaning
  line: "#CFC7B6"            # taupe hairline on paper — the only structural device
  white-overlay: "rgba(255,255,255,0.42)"
  # Product-surface tokens, for screenshots and any inset chrome. Never repaint these.
  ui-canvas: "#F6F6F8"
  ui-surface: "#FFFFFF"
  ui-ink: "#22252B"
  ui-blue: "#0D68D3"         # Agent 自主决定
  ui-red: "#B72A24"          # 约束冲突
  ui-green: "#189055"        # 受控检查通过

typography:
  # — reading ramp (-apple-system) —
  body:        { fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', 'PingFang SC', 'Microsoft YaHei', sans-serif", cqw: 1.0,  weight: 400, lineHeight: 1.6, color: "text-secondary" }
  body-sm:     { fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', 'PingFang SC', 'Microsoft YaHei', sans-serif", cqw: 0.85, weight: 400, lineHeight: 1.6 }
  subtitle:    { fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', 'PingFang SC', 'Microsoft YaHei', sans-serif", cqw: 1.3,  weight: 400, lineHeight: 1.5 }
  label:       { fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', 'PingFang SC', 'Microsoft YaHei', sans-serif", px: 14, weight: 500, tracking: "3px", upper: true, color: "accent" }
  attribution: { fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', 'PingFang SC', 'Microsoft YaHei', sans-serif", px: 15, weight: 400, tracking: "2px", upper: true, color: "accent" }
  micro:       { fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', 'PingFang SC', 'Microsoft YaHei', sans-serif", px: 12, weight: 400, tracking: "2px", upper: true, color: "accent" }
  # — display / hero ramp (system CJK stack, weight 600, sentence case) —
  # Hand-corrected: the preset's Playfair 400 became a 400-weight system sans on the swap,
  # which reads thin and grey at CJK display sizes. 600 matches the product's own
  # `font-weight: 600` display type (frontend/src/styles.css:584) and Apple's CJK title weight.
  # Negative tracking is applied per-ramp in the frames, not here.
  h3:          { fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', 'PingFang SC', 'Microsoft YaHei', sans-serif", cqw: 1.8, weight: 600, lineHeight: 1.1 }
  timeline-headline:{ fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', 'PingFang SC', 'Microsoft YaHei', sans-serif", cqw: 1.9, weight: 600, lineHeight: 1.1 }
  card-headline:{ fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', 'PingFang SC', 'Microsoft YaHei', sans-serif", cqw: 2.0, weight: 600, lineHeight: 1.15 }
  stat-figure: { fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', 'PingFang SC', 'Microsoft YaHei', sans-serif", cqw: 3.0, weight: 600, lineHeight: 1.0 }
  quote-mark:  { fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', 'PingFang SC', 'Microsoft YaHei', sans-serif", cqw: 9.0, weight: 400, lineHeight: 0.5, color: "line" }
  h2:          { fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', 'PingFang SC', 'Microsoft YaHei', sans-serif", cqw: 4.0, weight: 600, lineHeight: 1.1 }
  h1:          { fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', 'PingFang SC', 'Microsoft YaHei', sans-serif", cqw: 6.2, weight: 600, lineHeight: 1.06 }
  display:     { fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', 'PingFang SC', 'Microsoft YaHei', sans-serif", cqw: 8.0, weight: 600, lineHeight: 1.04 }
...
- **Fonts** — already set to **-apple-system** (display) / **Iowan Old Style** (body); ignore any preset font name lingering in prose.
...
The voice is a literary pairing: **-apple-system** at weight 400 (the thin-stroke didone, never
bold, always sentence case) carries every headline, numeral, and quote mark in ink; **-apple-system**
...
- **-apple-system 400** (ink, sentence case) for display; **-apple-system** body (gray) + labels (taupe, tracked).
...
Two ramps. The **reading ramp** (-apple-system body 1.0cqw gray, labels in px taupe) carries copy + chrome;
...
- **Playfair at 400, ink, sentence case** — never bold, never uppercase, never taupe. **-apple-system labels uppercase, 2–3px tracked, taupe.** Italic via Playfair italic for emphasis only.

## Depth & Surface

The flat plane is the only technique. Hierarchy from:

- **Type contrast** — Playfair serif vs -apple-system sans; the 8cqw→0.7cqw scale.
...
**Ground** `{colors.bg-primary}`, `pad-x`. **Composes** geo-ring (right, ~34cqw, solid+dashed), vertical-line (left), label, display/h1. **Focal** a 2–3 line Playfair `display`/`h1` headline in ink (italic on the key word), left-anchored, with a taupe `label` above and an -apple-system subtitle below. **Chrome** optional bottom meta row (Playfair value + taupe label). **Accent** the geo ring; optionally one `horizontal-accent` ink line (if it clears the meta). **Silence** ~55% empty. **Fixed** Playfair 400 ink sentence-case, ≤2 geo elements, 1px lines. **Free** title, ring placement, meta. **Density** sparse.
...
- **-apple-system + -apple-system via Google Fonts.** CJK pairing (Noto Serif SC 700/400) carries over; Playfair has no Hanzi italic — substitute weight/taupe for emphasis.
