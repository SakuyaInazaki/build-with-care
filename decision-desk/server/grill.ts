...
export async function nextGrill(
  run: RunState,
  settings: Settings,
  input: { round: number; answer?: string },
  ask = complete,
): Promise<GrillState> {
  ...
  if (state.status === 'question') {
    if (!input.answer?.trim() || !state.question) throw new Error('请选择选项或填写你的回答。')
    answers.push({ question: state.question.title, answer: input.answer.trim() })
  } else if (input.answer) throw new Error('还没有待回答的问题。')
